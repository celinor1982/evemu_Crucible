...
void MarketBotMgr::ForceRun() {
    std::printf("[%s] [MarketBot] ForceRun entered\n");
    std::fflush(stdout);

    if (!m_initalized) {
        std::printf("[%s] [MarketBot] MarketBotMgr not initialized — skipping run\n");
        std::fflush(stdout);
        return;
    }

    std::printf("[%s] [MarketBot] Running Process() now...\n");
    std::fflush(stdout);

    this->Process(true);  // force override

    std::printf("[%s] [MarketBot] Finished Process()\n");
    std::fflush(stdout);
...
void MarketBotMgr::Process(bool overrideTimer) {
    ...
    if (!overrideTimer && !m_updateTimer.Check()) {
        std::printf("[%s] [MarketBot] Update timer not ready yet.\n");
        std::fflush(stdout);
        _log(MARKET__TRACE, "Update timer not ready yet.");
        return;
    }
    
    std::printf("[%s] [MarketBot] Processing old orders...\n");
    std::fflush(stdout);
    _log(MARKET__TRACE, "Processing old orders...");
    ExpireOldOrders();

    std::vector<uint32> eligibleSystems = GetEligibleSystems();
    std::printf("[%s] [MarketBot] Found %zu eligible systems for order placement.\n", eligibleSystems.size());
    std::fflush(stdout);
    ...
    for (uint32 systemID : eligibleSystems) {
        std::printf("[%s] [MarketBot] Placing orders in systemID: %u\n", systemID);
        std::fflush(stdout);
    ...
    std::printf("[%s] [MarketBot] Master Summary: Created %d buy orders and %d sell orders across %zu systems. Removed %d old orders.\n",
    ...
    std::printf("[%s] [MarketBot] Cycle complete. Resetting timer.\n");
    std::fflush(stdout);
...
int MarketBotMgr::ExpireOldOrders() {
    ...
    if (!sDatabase.RunQuery(res,
        ...
        now, BOT_OWNER_ID)) {
        std::printf("[%s] [MarketBot] Failed to query expired bot orders.\n");
        std::fflush(stdout);
    ...
    while (res.GetRow(row)) {
        ...
        ++expiredCount;
        std::printf("[%s] [MarketBot] Expired MarketBot order %u\n", orderID);
        std::fflush(stdout);
...
int MarketBotMgr::PlaceBuyOrders(uint32 systemID) {
    ...
    if (!sDataMgr.GetSystemData(systemID, sysData)) {
        std::printf("[%s] [MarketBot] Failed to get system data for system %u\n", systemID);
        std::fflush(stdout);
    ...
    if (!sDataMgr.GetStationListForSystem(systemID, availableStations)) {
        std::printf("[%s] [MarketBot] No stations found for system %u\n", systemID);
        std::fflush(stdout);
    ...
    for (size_t i = 0; i < std::min<size_t>(stationLimit, sMBotConf.main.OrdersPerRefresh); ++i) {
        ...
        if (price * quantity > sMBotConf.main.MaxISKPerOrder) {
            if (quantity > 1) {
                ...
                if (price > sMBotConf.main.MaxISKPerOrder) {
                    std::printf("[%s] [MarketBot] Skipping itemID %u due to price %.2f ISK exceeding MaxISKPerOrder.\n", itemID, price);
                    std::fflush(stdout);
                    _log(MARKET__TRACE, "Skipping itemID %u due to price %.2f ISK exceeding MaxISKPerOrder.", itemID, price);
                    continue;
                }
                std::printf("[%s] [MarketBot] Price too high for bulk, retrying with quantity = 1 for itemID %u.\n", itemID);
                std::fflush(stdout);
                _log(MARKET__TRACE, "Price too high for bulk, retrying with quantity = 1 for itemID %u", itemID);
            } else {
                std::printf("[%s] [MarketBot] Skipping itemID %u even at quantity = 1 due to price %.2f ISK.\n", itemID, price);
                std::fflush(stdout);
        ...
        if (success) {
            ++orderCount;
            std::printf("[%s] [MarketBot] %s order created for typeID %u, qty %u, price %.2f ISK, station %u\n",
            ...
            std::printf("[%s] [MarketBot] Failed to store %s order for typeID %u at station %u\n",
    ...
    std::printf("[%s] [MarketBot] Created %d buy orders for system %u\n", orderCount, systemID);
    std::fflush(stdout);
    return orderCount;
}
...
int MarketBotMgr::PlaceSellOrders(uint32 systemID) {
    ...
    if (!sDataMgr.GetStationListForSystem(systemID, availableStations)) {
        std::printf("[%s] [MarketBot] No stations found for system %u — skipping order creation.\n", systemID);
        std::fflush(stdout);
        _log(MARKET__ERROR, "MarketBot: No stations found for system %u — skipping order creation.", systemID);
        return 0;
    } else {
        std::printf("[%s] [MarketBot] Found %zu stations in system %u\n", availableStations.size(), systemID);
        std::fflush(stdout);
    ...
    for (size_t i = 0; i < std::min<size_t>(stationLimit, sMBotConf.main.OrdersPerRefresh); ++i) {
        ...
        std::printf("[%s] [MarketBot] System %u maps to region %u via GetSystemData.\n", systemID, sysData.regionID);
        std::fflush(stdout);
    ...
    std::printf("[%s] [MarketBot] Created %d sell orders for system %u\n", orderCount, systemID);
    std::fflush(stdout);
    return orderCount;
}
...
std::vector<uint32> MarketBotMgr::GetEligibleSystems() {
    ...
    std::printf("[%s] [MarketBot] GetEligibleSystems(): Pulled %zu systems from StaticDataMgr\n", systemIDs.size());
    std::fflush(stdout);
...
uint32 MarketBotMgr::SelectRandomItemID() {
    ...
    do {
        ...
        std::printf("[%s] [MarketBot] Selected itemID %u after %u attempts.\n", itemID, tries);
        std::fflush(stdout);
    ...
    std::printf("[%s] [MarketBot] Failed to select valid itemID after %u attempts. Returning fallback itemID = 34 (Tritanium)\n", tries);
    std::fflush(stdout);
