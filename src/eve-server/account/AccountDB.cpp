...
// ---marketorder updates
double AccountDB::GetBalance(uint32 charID, uint16 accountKey) {
    if (IsCorp(charID)) {
        return GetCorpBalance(charID, accountKey);
    } else {
        return OfflineFundXfer(charID, 0.0, Account::CreditType::ISK); // Read-only, no change
    }
}

bool AccountDB::AlterBalance(uint32 charID, double amount, uint16 accountKey) {
    if (IsCorp(charID)) {
        double current = GetCorpBalance(charID, accountKey);
        UpdateCorpBalance(charID, accountKey, current + amount);
        return true;
    } else {
        OfflineFundXfer(charID, amount, Account::CreditType::ISK);
        return true;
    }
}

void AccountDB::RecordTransaction(
    uint32 fromID,
    uint32 toID,
    double amount,
    const char* reason,
    ...
    double toBalance = GetBalance(toID, toAccountKey);

    AddJournalEntry(fromID, entryType, fromID, toID, Account::CreditType::ISK, fromAccountKey, -amount, fromBalance, reason, refID);
    AddJournalEntry(toID,   entryType, fromID, toID, Account::CreditType::ISK, toAccountKey,  amount, toBalance, reason, refID);
}
// ---marketorder updates